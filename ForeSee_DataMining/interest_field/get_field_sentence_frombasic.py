...
def get_compNews(stockCode,researchTime,compDf,outpath1):
    ...
    while(i < len_sen):
        ...
        try:
            ...
            # tmp_list = [x for x in tmp_list if ((len(x) <5) & (len(x)>1))]
            field_tmp.extend(tmp_list)
            # stock_name_all.extend([stock_name]*find_sent_len)
            # research_time_all.extend([research_time]*find_sent_len)
...
if __name__ == '__main__':
    ...
    # sent_path = '/data/prj2020/EnterpriseSpider/news/basic_field_sent_0307v2.csv'
    # #field_sentence,label
    # pred_path = '/data/prj2020/EnterpriseSpider/news/pre0307v2.csv'
    # df1 = pd.read_csv(sent_path)
    # df2 = pd.read_csv(pred_path)
    # df1['label'] = df2['label']
    res_path = '/data/prj2020/EnterpriseSpider/news/predict_research_field_sent_0307v3.csv'
    # df1.to_csv(res_path,index=False,header=True)
    outpath1 = f'/data/prj2020/EnterpriseSpider/news/research_field/researchbasic_field_0307v3.csv'
    outpath2 = f'/data/prj2020/EnterpriseSpider/news/research_field/researchbasic_field_0307v3.json'
