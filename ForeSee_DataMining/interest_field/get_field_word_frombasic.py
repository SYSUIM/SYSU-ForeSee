...
def get_compNews(stockCode,compDf):
    ...
    research_time_all = []
    label_all = []
    outpath1 = f'/data/prj2020/EnterpriseSpider/news/basic_field_sent_0307v2.csv'

    # while(i < lenres):
    path = respath + f'{industry_code}/'+f'{stock_code}.json'
    try:
        with open(path, 'r',encoding='utf-8') as jsonfile:
            text = json.load(jsonfile)['scope']
        #定位到有顿号的句子
        senlist = re.split('[，,；;。！？\n\'\"]',text)
        find_sent=[sent for sent in senlist if '、' in sent]
        
        # pattern_sen = re.compile('[。;；！？]([^。；;！？\n]*[、]*[主营|主要|运营|项目|技术|业务|功能|主业|包括|营业|商品|产品|核心][^，。；\n].*?)[、，。；;]')
        # find_sent = re.findall(pattern_sen, text)
        ...
        research_time_all.extend(["2020"]*find_sent_len)
        label_all.extend([1]*find_sent_len)
...
if __name__ == '__main__':
    filepath = '/data/prj2020/EnterpriseSpider/basic/stock_index.csv'
