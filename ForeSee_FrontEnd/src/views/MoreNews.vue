            ...
            <div class="container">
                <div class="col-12 col-md-12">
                    <div class="d-flex bd-highlight mb-2">
                        <div class="title-page">企业新闻</div>
                    </div>
                    <!-- <div class="subtitle-1">
                        <span :data="company">{{ company }}</span> （股票代码：<span :data="stockCode"> {{ stockCode }}</span>）
                    </div> -->
                    <ol class="breadcrumb">
                      <li class="breadcrumb-item">共有  <span :data="totalRecords"> {{ totalRecords }} </span>  篇相关企业新闻</li>
                    </ol>
                </div>
            </div>
  ...
  methods: {
    async loadMoreNews(stockCode, page) {
      ...
      let { data } = await this.$get(
        "http://121.46.19.26:8288/ForeSee/companyNews/" + this.stockCode + "/" + page
      );
