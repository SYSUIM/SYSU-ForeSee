            ...
            <div class="container">
                <div class="col-12 col-md-12">
                    <div class="d-flex bd-highlight mb-2">
                        <div class="title-page">研究报告</div>
                    </div>
                    <!-- <div class="subtitle-1">
                        <span :data="company">{{ company }}</span> （股票代码：<span :data="stockCode"> {{ stockCode }}</span>）
                    </div> -->
                    <ol class="breadcrumb">
                      <li class="breadcrumb-item">共有  <span :data="totalRecords"> {{ totalRecords }} </span>  篇相关研究报告</li>
                    </ol>
                </div>
            </div>
        ...
        <el-table :data="newslist" class="table">
          ...
          <el-table-column prop="notice_title" label="标题">
            <template slot-scope="newslist">
              <a :href="newslist.row.research_link" target="_blank" class="TestCSS"
              >{{ newslist.row.research_title }}
              </a>
            </template>
          </el-table-column>
          <el-table-column prop="research_time" label="日期" width="180"></el-table-column>
        </el-table>
  ...
  methods: {
    async loadMoreNews(stockCode, page) {
      ...
      let { data } = await this.$get(
        "http://121.46.19.26:8288/ForeSee/research/" + this.stockCode + "/" + page
      );
      this.newslist = data.research;
